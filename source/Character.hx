...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		if (charLists.contains(character))
		{
			switch (curCharacter)
			{
				case 'boomBoxCHR':
					tex = Paths.getSparrowAtlas('characters/speakersChristmas', 'shared');
					frames = tex;
					animation.addByPrefix('idle', 'boombox', 24, false);
		...
		if (!isPlayer)
			{
				...
				if (curCharacter == 'dad')
					dadVar = 6.1;
				if (holdTimer >= Conductor.stepCrochet * 0.001 * dadVar)
				{
					dance();
					holdTimer = 0;
				}
